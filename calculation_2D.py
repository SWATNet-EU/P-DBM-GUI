...
class DBM2D:
    def __init__(
        ...
        if self.auto_dbm == False:
            ...
            self.gamma = (
                gamma
                * 1.0e-7  # manual drag parameter in unit of km-1. (typical value: 0.2e-7)
            )
        ...
        if self.P_DBM == True:
            ...
            if self.auto_dbm == False:
                try:
                    ...
                    self.dgamma = (
                        dgamma * 1.0e-7 if dgamma is not None else 0.1 * self.gamma
                    )
    ...
    def P_DBM_run(self):
        ...
        if self.auto_dbm == True and self.P_DBM == True:
            ...
            T_p10 = np.nanpercentile(T_array, 10, axis=0)
            T_p90 = np.nanpercentile(T_array, 90, axis=0)
            V_mean = np.nanmean(V_array)
            V_std = np.nanstd(V_array)
            V_p10 = np.nanpercentile(V_array, 10, axis=0)
            V_p90 = np.nanpercentile(V_array, 90, axis=0)
            ...
            results = {
                ...
                "Initial_time": self.time_utc.strftime("%d-%b-%Y %H:%M"),
            }
            ...
            gamma_array = np.clip(
                np.random.normal(self.gamma, self.dgamma / 3.0, self.N),
                1.0e-9,
                3.0e-7,
            )
            T_array, V_array, Hits, Miss, V0_array, Omega_array, Phi_CME_array = (
