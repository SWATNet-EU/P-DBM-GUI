...
def CME_edge_ensemble(
    ...
    # Compute 10–90% range
    tt_low, tt_high = np.percentile(tt_array, percentile_range)
    # Select only indices inside that range
    valid_idx = np.where((tt_array >= tt_low) & (tt_array <= tt_high))[0]
    idx = np.random.choice(valid_idx, size=num_samples, replace=False)
    ensemble_edges = []
    ensemble_angle = []
    ...
    return ensemble_angle, ensemble_edges, tt_array[idx]


def edge_visibility(tts, median, min_alpha=0.01, max_alpha=0.08):
    dist = np.abs(tts - median)
    max_dist = np.max(dist)
    if max_dist == 0:
        vis = np.full_like(tts, min_alpha)
    else:
        norm = 1 - dist / max_dist
        vis = min_alpha + (max_alpha - min_alpha) * norm
...
def setup_heliosphere(
    ...
    if pdbm == True:
        ...
        ax.axvline(np.deg2rad(np.median(phi_cme - omega)), color="r", ls="-")
        theta_ensemble, r_ensemble, t_calc = CME_edge_ensemble(
            tt, r0, v0, gamma, w, omega, phi_cme, cone_geometry, kinematic
        )
        tt_median = np.nanmedian(tt)
        viz = edge_visibility(t_calc, tt_median)
        for theta, r, a in zip(theta_ensemble, r_ensemble, viz):
            ax.plot(theta, r, color="r", alpha=a)
